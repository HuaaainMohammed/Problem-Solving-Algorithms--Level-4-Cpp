...
{
	//ابحث عن مكان كلمة StringToReplace داخل S1
	...
	while (pos != std::string::npos)
	{
		S1 = S1.replace(pos, StringToReplace.length(),sRepalceTo);
		 ...
		 // S1.replace(مكان البداية, كم حرف أحذف, ماذا أضع مكانه);
		pos = S1.find(StringToReplace);
	}
	return S1;
}

string FormateDate(stDate Date , string DateFormat = "yyyy/dd/mm")
{
...
int main()
{
	...
	cout << "\n" << FormateDate(Date, "Year:yyyy, Month:mm, Day:dd") << "\n";
